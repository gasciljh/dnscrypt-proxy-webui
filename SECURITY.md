...
>     in `docs/SECURITY.md` §5.30 and §17.1. They close edge cases
>     rather than fix known exploitable vulnerabilities.
...
>   • References to the full document updated to point at §5.30 and §17.1.
...
The vulnerability may be exploited before a fix is deployed. Use one of the
following channels:
...
These are documented in `docs/SECURITY.md` §5.30 and §17.1 as
**runtime improvements**, not audit corrections. They close edge
cases rather than fix known exploitable vulnerabilities.
...
- **Preserve settings on upgrade** — backup/restore of 5 user config files.
- **Custom Chains** — `DNSCRYPT_OUT` / `DNSCRYPT_OUT6` (no orphan rules).
